...
class Auth {
	...
	static function local_login_check() {
		...
		if($user_info['username'] == $username && $user_info['password'] == md5($password)) {
			...
			Session::set('userpath', $user_info['path']);
	...
	//添加、修改本地管理员
	static function add_user($name, $password, $path, $auth = array()) {
		if(empty($name) || empty($password)) {
			return FALSE;
		}
		$data = self::encode_user_data(array('username' => $name, 'password' => md5($password), 'path' => trim($path), 'auth' => $auth));
	...
	static function update_user_password() {
		...
		if(!empty($username) && $username == $userinfo['username']) {
			...
			if(self::add_user($userinfo['username'], $userinfo['password'], $userinfo['path'], $userinfo['auth'])) {
	...
	static function get_user_data($name) {
		...
		if(!is_array($data)) {
			$data = array('username' => 'XXX', 'password' => 'XXX', 'path' => 'XXX', 'auth' => 'XXX');
		}
