...
//登陆检测
Auth::is_login() || exit_feedback(301, '登陆超时或未登录！');

//权限检测
Auth::is_allow() || exit_feedback(300, '没有《'.strtoupper($module).'》模块操作权限,请联系系统管理员！');

//模块检测
NULL == C('ALLOWED_MODULE.*') && NULL == C('ALLOWED_MODULE.'.$module) &&
	exit_feedback(300, '《'.strtoupper($module).'》模块被禁用,请联系系统管理员！');
...
if('list' == $module) { //文件列表
	if(!is_dir(C('ROOT_PATH'))) {
		exit_feedback(300, C('ROOT_PATH').'  '.'无法访问初始根目录,请检查 "ROOT_PATH" 配置参数！');
	}
	$path            = array();
	...
	strlen($path['current']) < strlen($path['root']) &&
	...
	foreach($sdir as $val) {
		...
		if(in_array($dir, C('LIST_CONF.DISPLAY_NOTALLOW'))) {
			continue;
		}
	...
	foreach($sfile as $val) {
		...
		if(in_array($file, C('LIST_CONF.DISPLAY_NOTALLOW'))) {
			continue;
		}
...
//目录详情
	...
	$message = array(
		'green', '当前目录：', ''.substr($path, strlen(C('ROOT_PATH'))).'',
		'green', '目录详情：', '共'.$info['dir'].' 个目录，'.$info['file'].' 个文件',
		'green', '读写类型：', ''.($info['readable'] ? '可读, ' : '不可读, ').($info['writable'] ? '可写' : '不可写').'',
		'green', '总计大小：', ''.dealsize($info['size']).''
	);
	execute_feedback(200, $message);
	...
	if(file_exists($newname) || FALSE == file_exists($oldname)) {
		$statusCode = 300;
		$message    = array('blue', '命名失败：', '源文件不存在或新文件名和已文件有冲突');
	} else {
		if(rename($oldname, $newname)) {
			$statusCode = 200;
			$message    = array(
				'green', '原始名称：', ''.basename($_POST['oldname']).'',
				'green', '新文件名：', ''.basename($_POST['newname']).''
			);
		} else {
			$statusCode = 300;
			$message    = array('blue', '命名失败：', '错误原因未知');
		}
	}
	execute_feedback($statusCode, $message);
...
//ZIP解压
	...
	if(file_exists($path.$name) && is_file($path.$name)) {
		...
		if($result) {
			...
			$message = array(
				'green', '解压目标文件：', ' '.g2u($name).'',
				'green', '解压文件详情：', '共'.$fold.' 个目录，'.$fil.' 个文件',
				'green', '压缩文档大小：', ''.dealsize($tot_comp).'',
				'green', '解压文档大小：', ''.dealsize($tot_uncomp).''
			);
		} else {
			$statusCode = 300;
			$message    = array_merge($message, array('blue', '解压失败：', ''.$Zip->errorInfo(TRUE).''));
		}

	} else {
		$statusCode = 300;
		$message    = array('red', '无法解压:', '未知错误');
	}
	execute_feedback($statusCode, $message);
//ZIP压缩
} elseif('zip' == $module) {
	...
	if(!file_exists($zipname) && $Zip->create($files, PCLZIP_OPT_REMOVE_PATH, $path)) {
		...
		if($list) {
			...
			$message = array(
				'green', '压缩目标文件：', ' '.g2u(basename($zipname)).'',
				'green', '压缩文件详情：', '共'.$fold.' 个目录，'.$fil.' 个文件',
				'green', '压缩文档大小：', ''.dealsize($tot_comp).'',
				'green', '解压文档大小：', ''.dealsize($tot_uncomp).''
			);
		}
		...
		if(file_exists($zipname)) {
			$error = g2u(basename($zipname)).'已经存在！';
		} else {
			$error = $Zip->errorInfo(TRUE);
		}
		$message = array('blue', '压缩失败：', ''.$error.'');
	}
	execute_feedback($statusCode, $message);
	...
	foreach($files as $f) {
		$fs = $path.$f;
		if(empty($f) || !file_exists($fs)) {
			continue;
		}
		$info = array('dir' => 0, 'file' => 0);
		$err  = array('dir' => array(), 'file' => array());
		if(is_dir($fs) && File::del_dir($fs.'/', $info, $err)) {
			$ndir += $info['dirs'];
			$nfile += $info['files'];
			foreach(array_merge($err['dirs'], $err['files']) as $val) {
				$message = array_merge($message, array('blue', '删除失败：', ' '.g2u($val).''));
			}
		} elseif(is_file($fs) && unlink($fs)) {
			++$nfile;
		} else {
			$message = array_merge($message, array('blue', '删除失败：', ' '.g2u($f).''));
		}
	}
	$message = array_merge($message, array('green', '总计删除：', ''.$ndir.'目录，'.$nfile.'个文件'));
	execute_feedback(200, $message);
//文件移动、复制、粘贴
} elseif('paste' == $module) {
	...
	if($cut && 1 == count($files) && !file_exists($path_to.$files[0])) {
		...
		$message = array('green', '极速移动：', ''.($result ? '移动成功' : '移动失败').'');
		execute_feedback(200, $message);
	}
	//普通模式
	...
	$message = array('green', '目录变更：', ''.g2u($path_from).'  =>  '.g2u($path_to).'');
	if(!$cover && is_array($coverfiles) && !empty($coverfiles)) {
		foreach($coverfiles as $i) {
			...
			$message    = array_merge($message, array('blue', '覆盖文件：', ''.g2u($coverfile).''));
		}
	} else {
		$statusCode = 200;
		$result     = array();
		$message    = array_merge($message, array('green', '变更详情：', '共'.(($cut) ? '移动' : '复制').'目录'.$info['dirs'].'个,文件'.$info['files'].'个'));
		$message    = array_merge($message, array('green', '总计大小：', ''.dealsize($info['size']).''));
	}
	execute_feedback($statusCode, $message, $result);
//文件权限变更
} elseif('chmod' == $module) {
	if('show' == $action) {
		include(TPL_ROOT.'chmodfile.tpl.php');
		exit();
	} else {
		$files = explode('|', u2g(trim($_POST['files'])));
		...
		foreach($files as $f) {
			if($deep && is_dir($f)) {
				...
				foreach($err['dirs'] as $val)
					$message = array_merge($message, array('blue', '修改失败：', ' '.g2u($val).''));
				...
				if(FALSE == chmod($f, get_chmod($chmod))) {
					$message = array_merge($message, array('blue', '修改失败：', ' '.g2u($f).''));
				}
			}
		}
		$message = array_merge($message, array('green', '权限变更：', ' 0'.$chmod.''));
		$message = array_merge($message, array('green', '总计修改：', ''.$ndir.'个目录，'.$nfile.'个文件'));
		execute_feedback(200, $message);
	}
//新建目录、文件
	...
	if('file' == $type) {
		if(is_file($name)) {
			$statusCode = 300;
			$message    = array('blue', '新建失败：', '文件已存在');
		} else {
			file_put_contents($name, 'newfile at '.date('Y-m-d H:i:s'));
			if(fopen($name, 'a+')) {
				$statusCode = 200;
				$message    = array('green', '新建成功：', ''.$_POST['name'].'');
			} else {
				$statusCode = 300;
				$message    = array('blue', '新建失败：', '错误原因未知?');
			}
		}
	} elseif('dir' == $type) {
		if(is_dir($name)) {
			$statusCode = 300;
			$message    = array('blue', '新建失败：', '目录已存在');
		} else {
			if(mkdir($name, 0755)) {
				$statusCode = 200;
				$message    = array('green', '新建成功：', ''.$_POST['name'].'');
		...
		$statusCode = 300;
		$message    = 'Error:not exist this action!';
	}
	execute_feedback($statusCode, $message);
//图片缩略预览
...
//文本编辑
	...
	if('save' !== $action) {
		...
		!file_exists($file) && execute_feedback(300, array('green', '保存失败：', '目标文件不存在'));
		...
		if(!empty($newname)) {
			...
			file_exists($file) && execute_feedback(300, array('blue', '另存失败：', ''.g2u($file).'已存在'));
			$fp     = @fopen($file, 'w+');
			$result = file_put_contents($file, $code, LOCK_EX);
			if($fp && $result) {
				$statusCode = 200;
				$message    = array('green', '保存成功：', '文件已经保存');
			} else {
				$statusCode = 300;
				$message    = array('blue', '保存失败：', '错误原因未知');
			}
		} elseif(empty($newname) && file_put_contents($file, $code, LOCK_EX)) {
			$statusCode = 200;
			$message    = array('green', '保存成功：', '文件已经保存');
			$message    = array_merge($message, array('green', '编码变更：', ''.$oldcharset.'=>'.$charset.''));
		} else {
			$statusCode = 300;
			$message    = array('blue', '保存失败：', '错误原因未知');
		}
...
//批量上传文件
	...
	execute_feedback(300, array('green', '错误命令：', '未知API'));
}
//集中处理执行响应
function execute_feedback($statusCode, $message, $result = array()) {
	$msarr   = $message;
	$message = '';
	for($i = 0; $i < count($msarr); $i = $i) {
		$message .= '<font color="'.$msarr[$i++].'">'.$msarr[$i++].'</font><font color="red">'.$msarr[$i++].'</font><br />';
	}
...
//集中处理退出响应
function exit_feedback($code, $message) {
	if($isajax) {
		return_json($code, $message);
	} else {
		exit('<center><font color="red">'.$message.'</font></center>');
	}
}