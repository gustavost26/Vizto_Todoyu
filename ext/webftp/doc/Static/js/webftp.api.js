...
WebFTP.Api.actionMenu = function (type, option) {
	...
	if ('main' == type) {
		switch (option.action) {
			...
			case 'cut':
				WebFTP.Api.Cut(3, option);
				break;
			case 'copy':
				WebFTP.Api.Copy(3, option);
				break;
			case 'paste':
				WebFTP.Api.Paste(3, option);
				break;
			case 'delete':
				WebFTP.Api.Del(3, option);
				break;
			...
			case 'download':
				WebFTP.Api.Download(3, option);
				break;
			case 'selectall':
				WebFTP.Util.SelectAll();
				break;
		}
	} else if ('dir' == type) {
		switch (option.action) {
			case 'open':
				...
				break;
			case 'rename':
				WebFTP.Api.Rename(2, option);
				break;

			case 'cut':
				WebFTP.Api.Cut(2, option);
				break;
			case 'copy':
				WebFTP.Api.Copy(2, option);
				break;
			case 'delete':
				WebFTP.Api.Del(2, option);
				break;

			case 'chmod':
				WebFTP.Api.Chmod(2, option);
				break;

			case 'zip':
				WebFTP.Api.Zip(2, option);
				break;
			case 'download':
				WebFTP.Api.Download(2, option);
				break;
		}
	} else if ('file' == type) {
		switch (option.action) {
			case 'edit':
				WebFTP.Api.Edit(1, option);
				break;
			case 'rename':
				WebFTP.Api.Rename(1, option);
				break;

			case 'cut':
				WebFTP.Api.Cut(1, option);
				break;
			case 'copy':
				WebFTP.Api.Copy(1, option);
				break;
			case 'delete':
				WebFTP.Api.Del(1, option);
				break;

			case 'zip':
				WebFTP.Api.Zip(1, option);
				break;
			case 'unzip':
				WebFTP.Api.Unzip(1, option);
				break;
			case 'chmod':
				WebFTP.Api.Chmod(1, option);
				break;
			case 'view':
				WebFTP.Api.Viewzip(1, option);
				break;
			case 'download':
				WebFTP.Api.Download(1, option);
				break;
		}
	} else {
...
WebFTP.Api.Rename = function (type, option) {
	...
	if (1 == type || 2 == type) {
			...
			if (action == 'ok') {
				for (i = 1; i < szMsg.length + 1; i++) {
					if (val.indexOf(szMsg.substring(i - 1, i)) > -1) {
						asyncbox.tips('请勿输入非法字符如:' + szMsg, 'error');
						return;
					}
				}
				option.newname = val;
		...
		asyncbox.tips('Sorry,未知重命名操作！', 'error');
		return;
	}
}
//文件搜索
WebFTP.Api.Search = function (type, option) {
	asyncbox.tips('Sorry, 暂不支持文件搜索！', 'error');
	return;
}
//新建文件、目录
...
WebFTP.Api.Del = function (type, option, name) {
	...
	if (1 == type) {
		...
		if (1 > data.length) {
			asyncbox.tips('请至少选择一个文件或目录!', 'error');
			return;
		}
		var confirm_str = '';
...
WebFTP.Api.Zip = function (type, option) {
	...
	if (1 == type) {
		...
		asyncbox.tips('Sorry,未知压缩操作！', 'error');
		return;
	}
}
//文件剪切
...
WebFTP.Api.Copy = function (type, option) {
	...
	if (1 == type) {
		...
		if (0 < data.length) {
			...
			asyncbox.tips('Sorry,请至少选择一个文件!', 'error');
			return;
		}
	}
...
WebFTP.Api.Paste = function (type, option) {
	...
	if ($return) {
		...
		if (path_from == path_to) {
			asyncbox.tips('Sorry，源路径不能与目标路径相同!', 'error');
			return;
			...
			asyncbox.tips('Sorry, 剪贴板过期或没选择文件, 无法执行粘贴操作!', 'error');
			return;
		}
	}
}
...
WebFTP.Api.Loginout = function () {
	asyncbox.confirm('<font color="red">确定退出WebFTP ?</font>', '注销登录-' + WebFTP.poweredby, function (action) {
		if (action == 'ok') {
			document.location.href = './?m=login&a=out';
			return;
...
WebFTP.Api.Resetpass = function () {
	asyncbox.prompt('密码修改-' + WebFTP.poweredby, '请输入新密码:', '', 'text', function (action, val) {
		...
		if ($.trim(val).length < 4) {
			asyncbox.tips('Sorry, 密码长度不能小于4位!', 'error');
			return;
		}
	...
	if (1 == type) {
		...
		if (1 > selects.length && !option.file) {
			...
			asyncbox.open({
					...
					data: {
						module: 'chmod', action: 'show', files: encodeURI(files), chmod: option.chmod
					},
					tipsbar: {
						title: '目录文件权限修改', content: '目录文件权限修改...'
					},
					btnsbar: [
						{text: '提交更改', action: 'save_chmod' },
...
WebFTP.Api.Download = function (type, option, name) {
	...
	if (1 == type) {
		...
		asyncbox.tips('只支持单文件和单目录下载!', 'error');
		return;
	}
}
...
WebFTP.Api.Edit = function (type, option) {
	...
	if ('unknown' == WebFTP.Util.GetLanguage($(option.el).attr('fileext'))) {
		...
		var edit_width = WebFTP.Config.conf.edit.editor_conf.width || 900;
		var edit_height = WebFTP.Config.conf.edit.editor_conf.height || 600;
		asyncbox.open({
				id: 'editfile', url: 'Api.php', width: edit_width, height: edit_height,
				data: {
					module: 'editfile', action: 'show', file: file
				},
				title: '编辑：' + $(option.el).attr('filename'),
				tipsbar: {
					title: '文件编辑', content: '文件编辑器...'
				},
				...
				callback: function (action, iframe) {
					...
					if ('file_getcode' == action) {
						...
						iframe.view(WebFTP.Config.path);
						return false;
...
WebFTP.Api.Upload = function (type, option) {
	...
	asyncbox.open({
			...
			data: {
				'module': 'upload', 'action': 'show', 'path': WebFTP.Config.path.current
			},
			tipsbar: {
				title: '文件上传', content: '批量上传文件...'
			},
			btnsbar: [
				{text: '关闭窗口', action: 'close_window' }
			],
...
WebFTP.Api.Unzip = function (type, option) {
	...
	asyncbox.prompt('ZIP解压-' + WebFTP.poweredby, '请输入解压目录（解压到当前请输入 <font color="red">"."</font>）:', $(option.el).attr('filename').replace('.' + $(option.el).attr('fileext'), ''), 'text', function (action, val) {
		if (action == 'ok') {
			if ('' == val) {
				asyncbox.tips('Sorry,解压目录不能为空！', 'error');
				return;
			}
